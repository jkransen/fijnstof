...
object Fs2Sds011 extends IOApp {
  ...
  val converter: Stream[IO, String] = io.readInputStream(IO(is), 1, ec)
    .map(_.toInt & 0xff)
    .through(SdsStateMachine.collectMeasurements())
    .map(m => m.toString)

  override def run(args: List[String]): IO[ExitCode] = {
    converter.compile.fold(())((_, str) => println(str)).as(ExitCode.Success)
  }
}
...
object SdsStateMachine {
  ...
  def collectMeasurements[F[_]](): Pipe[F, Int, SdsMeasurement] = {
    ...
    def go(state: SdsState): Stream[F, Int] => Pull[F, SdsMeasurement, Unit] = _.pull.uncons1.flatMap {
